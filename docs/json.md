...
| `alwaysReinitializeMap` (optional) | boolean | Whether to reload the map frontend between each prompt. (The map is usually only reinitialized when the user jumps around.) Default: `false`
...
- The `layout` object has the following properties:
  ...
  | `hideScoringBreakdown` | boolean | Whether to hide the per-prompt scoring breakdown at the end of the game. Default: `false`
